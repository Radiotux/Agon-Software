// route.c - Utilidad nativa de tabla de rutas IP para Agon Light 2
// Basado estrictamente en la arquitectura no bloqueante de ifconfig.c de Radiotux

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdint.h>
#include <agon/mos.h>

#define BUFFER_SIZE 64

void wait_ms(uint32_t ms, volatile uint8_t *sv) {
    uint32_t start = *(volatile uint32_t *)(sv + 0x00);
    while ((*(volatile uint32_t *)(sv + 0x00) - start) < ms);
}

void flush_uart(void) {
    while (mos_ugetc_nb() >= 0);
}

void send_esp(const char *cmd) {
    while (*cmd) {
        mos_uputc(*cmd++);
    }
}

int scan_init_ok(volatile uint8_t *sv, uint32_t timeout_ms) {
    char match[] = "OK\r\n";
    int m_idx = 0;
    uint32_t start = *(volatile uint32_t *)(sv + 0x00);

    while ((*(volatile uint32_t *)(sv + 0x00) - start) < timeout_ms) {
        int c = mos_ugetc_nb();
        if (c >= 0) {
            if ((char)c == match[m_idx]) {
                m_idx++;
                if (match[m_idx] == '\0') {
                    return 1;
                }
            } else {
                m_idx = 0;
                if ((char)c == match[0]) { // Corregido: match -> match[0]
                    m_idx = 1;
                }
            }
        }
    }
    return 0;
}

int init_esp(volatile uint8_t *sv) {
    send_esp("+++");
    wait_ms(400, sv);
    flush_uart();
    send_esp("AT\r\n");
    if (!scan_init_ok(sv, 1500)) {
        return 0;
    }
    flush_uart();
    send_esp("ATE0\r\n");
    wait_ms(100, sv);
    flush_uart();
    return 1;
}

int extract_token_value(volatile uint8_t *sv, const char *token, char *output, uint32_t timeout_ms) {
    int t_len = strlen(token);
    int t_idx = 0;
    int o_idx = 0;
    uint32_t start = *(volatile uint32_t *)(sv + 0x00);

    while ((*(volatile uint32_t *)(sv + 0x00) - start) < timeout_ms) {
        int c = mos_ugetc_nb();
        if (c >= 0) {
            if (t_idx < t_len) {
                if ((char)c == token[t_idx]) {
                    t_idx++;
                } else {
                    t_idx = 0;
                    if ((char)c == token[0]) { // Corregido: token -> token[0]
                        t_idx = 1;
                    }
                }
            } else {
                if ((char)c == '"' || (char)c == '\r' || (char)c == '\n') {
                    output[o_idx] = '\0';
                    return 1;
                }
                if (o_idx < (BUFFER_SIZE - 1)) {
                    output[o_idx++] = (char)c;
                }
            }
        }
    }
    return 0;
}

int main(void) {
    UART settings;
    char local_ip[BUFFER_SIZE];
    char netmask[BUFFER_SIZE];
    char gateway[BUFFER_SIZE];
    char subred[BUFFER_SIZE];
    volatile uint8_t *sv = (volatile uint8_t *)mos_sysvars();

    strcpy(local_ip, "0.0.0.0");
    strcpy(netmask, "0.0.0.0");
    strcpy(gateway, "0.0.0.0");
    strcpy(subred, "0.0.0.0");

    settings.baudRate = 115200;
    settings.dataBits = 8;
    settings.stopBits = 1;
    settings.parity = 0;
    settings.flowcontrol = 0;
    settings.eir = 0;

    if (mos_uopen(&settings) != 0) {
        printf("Error: No se pudo abrir la UART1.\n");
        return 1;
    }

    if (!init_esp(sv)) {
        flush_uart();
        mos_uclose();
        printf("Error: No se pudo inicializar el ESP-01S (Comandos AT).\n");
        return 1;
    }

    send_esp("AT+CIPSTA?\r\n");

    // Extracción cronológica estricta según el volcado del firmware AT del ESP-01S
    extract_token_value(sv, "+CIPSTA:ip:\"", local_ip, 400);
    extract_token_value(sv, "+CIPSTA:gateway:\"", gateway, 400);
    extract_token_value(sv, "+CIPSTA:netmask:\"", netmask, 400);

    flush_uart();
    mos_uclose();

    // Cálculo pasivo de la subred local estimando los tres primeros octetos (Clase C común)
    int o1 = 0, o2 = 0, o3 = 0, o4 = 0;
    if (sscanf(local_ip, "%d.%d.%d.%d", &o1, &o2, &o3, &o4) == 4) {
        sprintf(subred, "%d.%d.%d.0", o1, o2, o3);
    }

    // Renderizado formal en pantalla de la tabla completa de rutas
    printf("\nTabla de enrutamiento IP de Agon (ESP-01S a 115200 bps)\n");
    printf("====================================================================\n");
    printf("%-18s %-18s %-18s %-10s\n", "Destino de Red", "Pasarela (GW)", "Genmask", "Interfaz");
    printf("--------------------------------------------------------------------\n");

    // Fila 1: Ruta predeterminada global (0.0.0.0) apuntando a la pasarela real de tu hogar
    printf("%-18s %-18s %-18s %-10s\n", "0.0.0.0", gateway, netmask, "wlan0");

    // Fila 2: Ruta de enlace de datos local directa (Segmento de red local)
    if (strcmp(subred, "0.0.0.0") != 0) {
        printf("%-18s %-18s %-18s %-10s\n", subred, "0.0.0.0", netmask, "wlan0");
    }
    printf("====================================================================\n");

    return 0;
}
